...
public class DbData : DbContext
{
    ...
    {
        var dt = new DataTable();
        string sql = @"SELECT p.Name AS `Prod. Manager`, v.Name AS `Vendor Name`, v.Location AS `Vendor Location`, v.Phone AS `Vendor Phone`, w.Cases025Ltr AS `0.25L`, w.Cases05Ltr AS `0.5L`, w.Cases1Ltr AS `1L`, w.Cases2Ltr AS `2L`, w.Cases5Ltr AS `5L`, w.Cases20Ltr AS `20L`, w.AmountBilled AS `Amount Billed`, w.AmountPaid AS `Amount Paid`, (w.AmountBilled - w.AmountPaid) AS `Debt`, w.BillingDate AS `BillingTime`, CASE w.OrderStatus WHEN 1 THEN 'Pending' WHEN 2 THEN 'Collected' WHEN 3 THEN 'Return' ELSE 'Unknown' END AS 'OrderStatus' FROM WaterCanOrders w INNER JOIN ProdManagers p ON w.ProdManagerId = p.ProdManagerId INNER JOIN Vendors v ON w.VendorId = v.VendorId INNER JOIN SalesPersons s ON s.SalesPersonId = w.SalesPersonId WHERE p.ProdManagerId = @ProdManagerId";
        using (var conn = new MySqlConnection(_context.Database.GetDbConnection().ConnectionString))
        {
            try
            {
